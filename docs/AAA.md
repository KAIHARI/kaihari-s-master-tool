...
67b. ~~**Something on the desk that answers a finger.**~~ **Done, and rebuilt.** The Millennium Puzzle: a four-sided turn — a pyramid, apex down — with a chamfer round its top edge and a torus bail standing on it, gold, propped thirty-four degrees back on the bare left of the desk opposite the lamp. Tap it and it turns a third of a turn and rises; nothing else in the room moves, ever. Five things it established that a second prop inherits. A prop is a **pose** rather than a `ScenePiece`, because furniture is solved twice a day and a moving thing cannot live in a value that is deliberately recomputed. It **is propped**, and that is what makes it read at all: balanced on its point an inverted pyramid is occluded by its own top face from every seat, and the object comes out as a flat gold square with a ring lying on it — shot both ways before deciding. Where it rests is **solved per pose**, because `Rot3` spins about the object's own axis before tipping it, so which corner is lowest changes with the turn; one dropped height sank it three quarters of a pixel into the wood on every nudge. It is hit-tested against its **flattened silhouette**, not its footprint: the middle of its top face lands 98px from its contact point overhead, 140px at the table and 188px seated, against cards 104px wide, so the footprint is not even adjacent to the answer. And it is **two shapes and two parts**: it joins `ScenePainter`'s sort as its reach box and is drawn from its live faces — which is what lets the painter stay ignorant of any shape without axes — while its body and its bail are ordered among themselves by that same painter. Painting the whole thing last instead is what the first version did, and it assumed a camera in front of the table; yaw is free, and past about 145 degrees you are behind the room's own wall, where the header above the window is nearer than the desk. Seated at 150 it drew through it over 269 by 280 pixels. It carries **no mark**: an Eye of Wdjat was cast into its front flank and cut — `docs/LOOP.md` iteration 12 has why, and what a second attempt would need.

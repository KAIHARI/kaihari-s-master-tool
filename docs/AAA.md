...
61d. **Nothing in the room casts a shadow** — *and the decision this was waiting on has been made.* The lamp does not and the desk does not; shadows on this stage belong to the cards, which is what the stage is about. That was a defensible line and not an oversight, but it was the line: a single object throwing a shadow onto a desk where nothing else does does not read as better lighting, it reads as the one thing that got special treatment. (It is also why the puzzle shipped without one, before it was cut.) The machinery is all there — `Shadows.landOn` projects any set of corners onto any plane, and the night key has a real position — so this was a decision to revisit as a set, with the lamp, rather than a thing to bolt onto one object. **kai has asked for the set.** Three things follow from *how* it has to be built, and none of them is optional. There is no depth buffer and there cannot be one: paint order in the room is `ScenePainter`'s **separating axis**, and a depth buffer that could disagree with it is a bug class with no test that can catch it. So a room shadow is a **decal on a receiver's face**, painted inside `drawSolid` with the piece it lands on, and it never enters the piece ordering. And because furniture is solved twice a day and remembered, the projection belongs at solve time rather than in a frame — which is also the only version of this that survives the room growing past twenty pieces. The daytime half of it is downstream of #61c: a room shadow needs a **sun** to be cast by, and the day key is one broad light until that split happens.

...
61d. **Nothing in the room casts a shadow.** The lamp does not, the desk does not, and the puzzle standing beside the mat does not either — shadows on this stage belong to the cards, which is what the stage is about. That is a defensible line and not an oversight, but it is the line, and it is why the puzzle was shipped without one: a single object throwing a shadow onto a desk where nothing else does does not read as better lighting, it reads as the one thing that got special treatment. The machinery is all there — `Shadows.landOn` projects any set of corners onto any plane, and the night key has a real position — so this is a decision to revisit as a set, with the lamp, rather than a thing to bolt onto one prop.

67b. ~~**Something on the desk that answers a finger.**~~ **Done, once.** The Millennium Puzzle: a truncated pyramid apex-down, gold, standing on the bare left of the desk opposite the lamp. Tap it and it turns a third of a turn and rises; nothing else in the room moves, ever. Three things it established that a second prop inherits. A prop is a **pose** rather than a `ScenePiece`, because furniture is solved twice a day and a moving thing cannot live in a value that is deliberately recomputed. It may spin and rise and may **not** tumble — `CardSolid.slab` hangs a body along the stage's z, so a turn about that axis is exact and a tilt would shear the whole silhouette. And it is hit-tested against its **flattened silhouette**, not its footprint: at the table seat the middle of its top face is 102px from the foot it stands on, against cards 104px wide, so the footprint is not even adjacent to the answer. And it is **two shapes**: it joins `ScenePainter`'s sort as its reach box and is drawn from its live faces, which is what lets the painter stay ignorant of any shape without axes — a bounding box that shares no volume with anything is as good a separating axis as the real solid. Painting it last instead is what the first version did, and it assumed a camera in front of the table; yaw is free, and past about 145 degrees you are behind the room's own wall, where the header above the window is nearer than the desk. Seated at 150 it drew through it over 269 by 280 pixels.

67c. **The puzzle is a flat gold square, and the geometry is not the reason.** Found the first time anybody rendered a frame and looked at it. An apex-down pyramid standing on a table is, from a camera that looks *down* at that table, its own top face and almost nothing else: measured on a 1600x856 board, the body is **0% of the silhouette overhead, 2% at the table seat and 10% seated**. Nothing is wrong with the solid — `CardSolid.slab` builds exactly the shape it is asked for, and every test passes — it is the same trap `docs/AAA.md` #61's height rule describes and that `CardSolid.pileDepth` already exists to work around: on this stage a height is worth `sin(tilt)` of itself, and a shape whose whole character is its height loses that character. Proportion cannot rescue it; the body's share scales with tall-over-wide, so reading as a pyramid would need a spike four or five times its own width. Stood the other way up — a small top face over a wide base, which is one inverted `backScale` — the body is **99% at every seat** and it reads as a solid at a glance. That is a decision about what the object *is* rather than about how it is drawn, because the Millennium Puzzle's whole silhouette is that it hangs point-down, so it is recorded here rather than taken.

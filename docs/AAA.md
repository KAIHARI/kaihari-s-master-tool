...
61d. **Nothing in the room casts a shadow.** The lamp does not and the desk does not — shadows on this stage belong to the cards, which is what the stage is about. That is a defensible line and not an oversight, but it is the line: a single object throwing a shadow onto a desk where nothing else does does not read as better lighting, it reads as the one thing that got special treatment. (It is also why the puzzle shipped without one, before it was cut.) The machinery is all there — `Shadows.landOn` projects any set of corners onto any plane, and the night key has a real position — so this is a decision to revisit as a set, with the lamp, rather than a thing to bolt onto one object.
